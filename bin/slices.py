        ...
        for pdb_code,cen,lin,pla,atoms in coords_list:
            ...
            if not ml.success():
                ...
                if "2d" in d2or3:
                    ...
                    if d2or3 == "2d_np":
                        # 2d plot (s)
                        filename2 = f"2d_matrices_{pdb_code}_{count}"
                        np.save(filename2,vals2d.get_as_np())                    
                    elif d2or3 == "2d_html":
                        filename2 = f"2d_html_{pdb_code}_{count}.html"
                        naybs = mf.get_slice_neighbours(cc,ll,pp,width,samples,[0,1],log_level=1)                        
                    ...
                    if d2or3 == "3d_np":
                        filename3 = f"3d_matrices_{pdb_code}_{count}"                                                                        
                        np.save(filename3,vals3d.get_as_np())                                                        
                    elif d2or3 == "3d_html":
                        filename3 = f"3d_html_{pdb_code}_{count}.html"                    
